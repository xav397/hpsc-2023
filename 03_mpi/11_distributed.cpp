...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], tmp[N/size];
  srand48(rank);
  ...
  MPI_Win win;
  MPI_Win_create(tmp, N/size*sizeof(Body), sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  ...
  for(int irank=0; irank<size; irank++) {
    ...
    MPI_Win_fence(0, win);
    std::memcpy(&jbody, &tmp, N/size*sizeof(Body));
